...
class ECDSep_F0tuning(Optimizer):
    '''
    Optimizer based on the new separable Hamiltonian and generalized bounces
    lr (float): learning rate, called Delta t in the paper (required)
    F0 (float): expected minimum of the objective
    deltaEn (float): initial energy
    eta (float): hyperparameter that controls the concentration of the measure. Has to be an odd integer when F0_tuning is true. (required)
    consEn (bool): whether the energy is conserved or not
    weight_decay (float): weight decay, implemented as L^2 term
    nu (float): chaos hyperparameter
    s (float): regularization switch
    F0_tuning : set to True to use the self-tuning of F0 (experimental)
    '''

    def __init__(self, params, lr=required, F0=0., eps1=1e-10, eps2=1e-40, deltaEn=0., nu=1e-5, s=1., weight_decay=0, eta=required, consEn=True, F0_tuning = False):
        defaults = dict(lr=lr, F0=F0, eps1=eps1, eps2=eps2, deltaEn=deltaEn, nu=nu, s=s, weight_decay=weight_decay, eta=eta, consEn=consEn, F0_tuning = F0_tuning )
        self.F0 = F0
        self.s = s
        if self.s == 1:
            self.deltaEn = deltaEn
        ...
        self.weight_decay = weight_decay
        self.nu = nu
        self.F0_tuning = F0_tuning
        self.F0_shift = 0.0
        super(ECDSep_F0tuning, self).__init__(params, defaults)
    ...
    def step(self, closure):
        ...
        V = (loss + 0.5*self.weight_decay*self.q2 -(self.F0+self.F0_shift))**self.eta
        ...
        if self.iteration == 0:
            ...
            if self.F0_tuning == True:
                print("Warning, self tuning of F0 is experimental!")

            
        if V > (not self.F0_tuning)*self.eps2:
            ...
            self.iteration += 1
                ...
                self.F0_shift = self.F0_shift+5*V
                print("Shifting F0, remember this is experimental!")
                print("New F0: ", (self.F0_shift+self.F0).item() )
